...
namespace dwarf
{
    ...
    private:
		...
		std::vector<std::uint8_t*> ipstack;

		std::size_t addressSize;
		std::size_t dwarfWidth;

        std::int64_t(*readRegister)(std::uint64_t);
        std::int64_t(*getFrameBase)();
        std::int64_t(*readAddress)(std::uint64_t);
        std::int64_t(*readAddressEx)(std::uint64_t, std::uint64_t);
        std::int64_t(*getObjectAddress)();
        std::int64_t(*getTLSAddress)(std::uint64_t);
        std::int64_t(*getCallFrameCFA)();
    ...
    private:
        std::int64_t runNext(OpCode code, const std::uint8_t* operands, std::size_t length)
        {
            auto intcode = static_cast<std::uint32_t>(code);

            // Handle push literal
            if (code >= OpCode::Lit0 && code <= OpCode::Lit31) {
                stack_push(intcode - static_cast<std::uint32_t>(OpCode::Lit0));
            }
            // Handle push address
            else if (code == OpCode::Address) {
                std::uint64_t value; std::memcpy(&value, operands, addressSize);
            ...
            {
                auto size = 1 << ((intcode - static_cast<std::uint32_t>(OpCode::Const1U)) / 2);
                std::uint64_t value; std::memcpy(&value, operands, size);
            ...
            {
                auto size = 1 << ((intcode - static_cast<std::uint32_t>(OpCode::Const1S)) / 2);
                std::int64_t value; std::memcpy(&value, operands, size);
                // Perform manual sign extension
                if (value & (1 << (size * 8 - 1))) { value |= static_cast<std::uint64_t>(-1) << (size * 8); }
                stack_push(value); return size;
            }
            // Handle push ULEB constant
            else if (code == OpCode::ConstU)
            {
                std::uint64_t value; auto size = uleb_read(operands, value);
                stack_push(value); return size;
            }
            // Handle push LEB constant
            else if (code == OpCode::ConstS)
            {
                std::int64_t value; auto size = sleb_read(operands, value);
                stack_push(value); return size;
            }
            ...
            {
                std::uint64_t reg; auto size = uleb_read(operands, reg);
                std::int64_t value; size += sleb_read(operands + size, value);
            ...
            // Handle stack operations
                ...
                std::uint8_t index = operands[0]; stack_push(stack_at(index)); return 1;
            }
            ...
            // Handle external value operations
                ...
                auto value = readAddress(static_cast<std::uint64_t>(stack.front()));
                stack_pop(); stack_push(value);
            }
            else if (code == OpCode::DerefSize) {
                std::uint8_t vsize = operands[0];
                std::uint64_t i1 = static_cast<std::uint64_t>(stack.front()); stack_pop();
                std::uint64_t value = readAddress(i1);
                value &= ~(static_cast<std::uint64_t>(-1) << (vsize * 8));
                stack_push(value);
                return 1;
            }
            else if (code == OpCode::XDeref) {
                auto i1 = static_cast<std::uint64_t>(stack.front()); stack_pop();
                auto i2 = static_cast<std::uint64_t>(stack.front()); stack_pop();
                ...
                auto i1 = static_cast<std::uint64_t>(stack.front()); stack_pop();
                auto i2 = static_cast<std::uint64_t>(stack.front()); stack_pop();
                std::uint64_t value = readAddressEx(i1, i2);
                value &= ~(static_cast<std::uint64_t>(-1) << (vsize * 8));
                stack_push(value);
                return 1;
            }
            ...
            // Handle arithmetic/logic operations
                ...
                auto i1 = static_cast<std::int64_t>(stack.front()); stack_pop();
                auto i2 = static_cast<std::int64_t>(stack.front()); stack_pop();
                stack_push(i2 - i1);
            }
			...
			{
				std::int16_t offset; std::memcpy(&offset, operands, 2);
				return 2 + offset;
			}
			else if (code == OpCode::Bra)
			{
				...
				if (i1) { std::memcpy(&offset, operands, 2); }
				return 2 + offset;
			}
